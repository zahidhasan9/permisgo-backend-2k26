...
export const getNearbyTeachers = asyncHandler(async (req, res) => {
  ...
  const [profiles, vehicles, availabilities, bookings, completedLessons] = await Promise.all([
    TeacherProfile.find({
      user: { $in: teacherIds },
      verificationStatus: "verified",
      availabilityStatus: "available",
    ...
    Lesson.find({ teacher: { $in: teacherIds }, status: "completed" })
      .select("teacher duration")
      .lean(),
  ]);

  const workedMinutesByTeacher = new Map();
  completedLessons.forEach((lesson) => {
    const key = String(lesson.teacher);
    workedMinutesByTeacher.set(key, (workedMinutesByTeacher.get(key) || 0) + Number(lesson.duration || 0));
  });
  ...
  const results = profiles
    ...
    .map((profile) => {
      ...
      return {
        ...profile,
        hoursWorked: Math.round(((workedMinutesByTeacher.get(teacherId) || 0) / 60) * 10) / 10,
        vehicles: teacherVehicles,
        nearestLocation: location,
        locations: [location],
        distanceKm: Number(location.distanceKm.toFixed(2)),
