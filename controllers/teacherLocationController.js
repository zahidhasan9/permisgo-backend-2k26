...
export const getNearbyTeachers = asyncHandler(async (req, res) => {
  ...
  const [profiles, vehicles, availabilities, bookings, completedLessons] =
    await Promise.all([
      TeacherProfile.find({
        user: { $in: teacherIds },
  ...
  completedLessons.forEach((lesson) => {
    ...
    workedMinutesByTeacher.set(
      key,
      (workedMinutesByTeacher.get(key) || 0) + Number(lesson.duration || 0),
    );
  });
  ...
  const results = profiles
    ...
    .map((profile) => {
      ...
      return {
        ...profile,
        hoursWorked:
          Math.round(((workedMinutesByTeacher.get(teacherId) || 0) / 60) * 10) /
          10,
