...
const cleanPayload = (body) => ({
  ...
  translations: {
    bn: {
      question: String(body.question_bn || "").trim(),
      answer: String(body.answer_bn || "").trim(),
    },
    fr: {
      question: String(body.question_fr || "").trim(),
      answer: String(body.answer_fr || "").trim(),
    },
  },
});
...
const localize = (item, lang) => {
  ...
  return {
    ...data,
    question: value.question || data.question,
    answer: value.answer || data.answer,
    language: lang,
  };
};
...
export const getFaqs = asyncHandler(async (req, res) => {
  ...
  sendResponse(
    ...
    200,
    "FAQs fetched.",
    faqs.map((item) => localize(item, req.query.lang)),
  );
});
