...
export const createContactSubmission = asyncHandler(async (req, res) => {
  const payload = {
    ...
    subject: clean(req.body.subject), location: clean(req.body.location),
    description: clean(req.body.description),
  };
  if (Object.values(payload).some((value) => !value)) throw new ApiError(400, "All contact fields are required.");
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payload.email)) throw new ApiError(400, "Please enter a valid email address.");

  const submission = await ContactSubmission.create(payload);
  const recipientSetting = await Setting.findOne({ key: "contactRecipientEmail" }).lean();
  const recipient = clean(recipientSetting?.value || process.env.CONTACT_RECIPIENT_EMAIL || process.env.SMTP_USER);
  if (recipient) {
    sendEmail({
      to: recipient,
...
export const getContactSubmissions = asyncHandler(async (req, res) => {
  ...
  sendResponse(res, 200, "Contact submissions fetched.", { items, total, page, pages: Math.max(1, Math.ceil(total / limit)) });
});

export const updateContactStatus = asyncHandler(async (req, res) => {
  if (!["new", "read", "resolved"].includes(req.body.status)) throw new ApiError(400, "Invalid contact status.");
  const item = await ContactSubmission.findByIdAndUpdate(req.params.id, { status: req.body.status }, { returnDocument: "after", runValidators: true });
