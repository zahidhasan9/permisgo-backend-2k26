...
export const listAllStudentLessons = async (req, res) => {
  try {
    const lessons = await EbookLesson.find({ status: "active" })
      .select("title subtitle course topic order createdAt")
      .populate({ path: "course", match: { status: "active" }, select: "title order" })
      .populate({ path: "topic", match: { status: "active" }, select: "title order" })
      .sort({ order: 1, createdAt: 1 })
      .lean();
    const visible = lessons.filter((lesson) => lesson.course && lesson.topic);
    const progressList = await EbookLessonProgress.find({
      student: req.userId,
      lesson: { $in: visible.map((lesson) => lesson._id) },
    }).lean();
    const progressByLesson = new Map(progressList.map((progress) => [String(progress.lesson), progress]));
    res.json({
      success: true,
      data: visible.map((lesson) => ({
        ...
        topicId: lesson.topic._id,
        category: lesson.topic.title,
        progress: progressByLesson.get(String(lesson._id)) || null,
      })),
    });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
