...
export const createExamQuestion = asyncHandler(async (req, res) => {
  ...
  const question = await ExamQuestion.create({ number: Number(req.body.number), title: req.body.title || "", category: req.body.category || "General", image: image ? getUploadedFileUrl(image) : req.body.image || "", videoUrl: req.body.videoUrl || "", items: parseItems(req.body.items), order: Number(req.body.order) || 0, status: req.body.status || "active", createdBy: req.user._id });
