...
const fileFor = (req, field) =>
  Array.isArray(req.files)
    ? req.files.find((file) => file.fieldname === field)
    : null;
const parseItems = (value) => {
  ...
  if (typeof value === "string") {
    try {
      ...
      throw new ApiError(400, "Question items must be valid JSON.");
    }
  }
  if (!Array.isArray(items))
    throw new ApiError(400, "At least one question and answer is required.");
  const cleaned = items
    .map((item) => ({
  ...
  if (!cleaned.length || cleaned.length > 20)
    throw new ApiError(
      400,
      "Provide between 1 and 20 complete question-answer pairs.",
    );
  return cleaned;
};

export const getStudentExamQuestions = asyncHandler(async (req, res) => {
  const questions = await ExamQuestion.find({ status: "active" })
    .select("number title category image videoUrl items order")
...
export const getAdminExamQuestions = asyncHandler(async (req, res) => {
  ...
  sendResponse(res, 200, "Exam questions fetched.", items, {
    page,
    limit,
    total,
...
export const createExamQuestion = asyncHandler(async (req, res) => {
  ...
  if ((!image && !videoUrl) || (image && videoUrl)) {
    throw new ApiError(
      400,
      "Choose exactly one question media: image or video.",
    );
  }
  const question = await ExamQuestion.create({
    ...
    image: image ? getUploadedFileUrl(image) : "",
    videoUrl,
...
export const updateExamQuestion = asyncHandler(async (req, res) => {
  ...
  ["title", "category", "videoUrl", "status"].forEach((field) => {
  ...
  if (
    (!question.image && !question.videoUrl) ||
    (question.image && question.videoUrl)
  ) {
    throw new ApiError(
      400,
