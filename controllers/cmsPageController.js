...
const cleanSlug = (value) => String(value || "home").trim().toLowerCase().replace(/^\/+|\/+$/g, "") || "home";
const cleanTranslation = (value = {}) => ({
  ...
  keywords: Array.isArray(value.keywords) ? value.keywords.map(String).map((item) => item.trim()).filter(Boolean) : String(value.keywords || "").split(",").map((item) => item.trim()).filter(Boolean),
...
const localized = (page, language) => {
  ...
  const translation = primary.title
    ? { ...fallback, ...primary, settings: { ...(fallback.settings || {}), ...(primary.settings || {}) } }
    : fallback;
  return { ...item, language: requested, translation };
};

export const getPublicPage = asyncHandler(async (req, res) => {
  const page = await CmsPage.findOne({ slug: cleanSlug(req.params.slug), status: "published" });
...
export const getPublicSitemapPages = asyncHandler(async (_req, res) => {
  const pages = await CmsPage.find({ status: "published", noIndex: false }).select("slug translations updatedAt").lean();
...
export const getFooterPages = asyncHandler(async (req, res) => {
  ...
  sendResponse(res, 200, "Footer pages fetched.", pages.map((page) => {
    const item = localized(page, req.query.lang);
    return { slug: item.slug, title: item.translation?.title || item.slug, footerSection: item.footerSection, footerOrder: item.footerOrder };
  }));
});
...
export const upsertPage = asyncHandler(async (req, res) => {
  ...
  const translations = {};
  for (const language of languages) translations[language] = cleanTranslation(req.body.translations?.[language]);
  if (!translations.en.title) { res.statusCode = 400; throw new Error("English title is required."); }
  const page = await CmsPage.findOneAndUpdate(
    { slug },
    {
      ...
      footerSection: ["about", "partnership", "services", "support"].includes(req.body.footerSection) ? req.body.footerSection : "services",
      footerOrder: Number.isFinite(Number(req.body.footerOrder)) ? Number(req.body.footerOrder) : 0,
      pageTemplate: ["modern", "classic", "minimal"].includes(req.body.pageTemplate) ? req.body.pageTemplate : "modern",
      accentColor: /^#[0-9a-f]{6}$/i.test(String(req.body.accentColor || "")) ? req.body.accentColor : "#123f88",
      contentAlignment: req.body.contentAlignment === "center" ? "center" : "left",
