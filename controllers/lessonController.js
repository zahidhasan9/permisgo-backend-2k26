...
export const startLesson = asyncHandler(async (req, res) => {
  ...
  lesson.attendance.studentConfirmed = true;
  lesson.attendance.teacherConfirmed = true;
  lesson.attendance.studentConfirmedAt = completedAt;
  lesson.attendance.teacherConfirmedAt = completedAt;
  lesson.lessonProgress.studentConfirmedAt = completedAt;
  addHistory(lesson, "lesson_started_and_completed", req.user, "Lesson completed with automatic attendance.");
  await lesson.save();
  ...
  await createNotificationsSafely({
    user: lesson.student,
    title: "Lesson completed",
    message: "Your instructor completed the lesson and attendance was recorded automatically.",
    type: "lesson",
    actionUrl: "/student/lessons",
  });

  const updatedLesson = await populateLesson(Lesson.findById(lesson._id));
  return sendResponse(res, 200, "Lesson completed and attendance recorded successfully.", updatedLesson);
});
...
export const submitLessonFeedback = asyncHandler(async (req, res) => {
  ...
  const latestCompletedLesson = await Lesson.findOne({
  ...
  if (getId(latestCompletedLesson) !== getId(lesson)) {
    throw new ApiError(400, "Only your latest completed lesson can be reviewed.");
  }

  if (lesson.lessonProgress?.feedbackSubmittedAt) {
    throw new ApiError(400, "This lesson has already been reviewed.");
  }
  ...
  const teacherRating = await Review.aggregate([
    { $match: { teacher: lesson.teacher, status: "visible" } },
  ...
  await TeacherProfile.findOneAndUpdate(
    ...
    {
      $set: {
        "rating.average": Number(teacherRating[0]?.average || 0),
        "rating.totalReviews": Number(teacherRating[0]?.totalReviews || 0),
      },
    },
  );
...
export const getLatestLessonForFeedback = asyncHandler(async (req, res) => {
  const lesson = await populateLesson(
    Lesson.findOne({ student: req.user._id, status: "completed" }).sort({
      completedAt: -1,
      updatedAt: -1,
      _id: -1,
    }),
  ...
  return sendResponse(res, 200, "Latest completed lesson fetched.", {
    lesson,
    canReview: Boolean(lesson && !lesson.lessonProgress?.feedbackSubmittedAt),
  });
});
