...
export const startLesson = asyncHandler(async (req, res) => {
  ...
  addHistory(
    lesson,
    "lesson_started_and_completed",
    req.user,
    "Lesson completed with automatic attendance.",
  );
  await lesson.save();
  ...
  await createNotificationsSafely({
    ...
    message:
      "Your instructor completed the lesson and attendance was recorded automatically.",
    type: "lesson",
    actionUrl: "/student/lessons",
  });
  ...
  return sendResponse(
    res,
    200,
    "Lesson completed and attendance recorded successfully.",
    updatedLesson,
  );
});
...
export const submitLessonFeedback = asyncHandler(async (req, res) => {
  ...
  if (getId(latestCompletedLesson) !== getId(lesson)) {
    throw new ApiError(
      400,
      "Only your latest completed lesson can be reviewed.",
    );
  }
