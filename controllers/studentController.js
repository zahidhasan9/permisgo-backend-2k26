...
export const getMyBookletSkills = asyncHandler(async (req, res) => {
  const assessments = await StudentSkillAssessment.find({
...
export const getMyFavoriteTeachers = asyncHandler(async (req, res) => {
  ...
  const profiles = await TeacherProfile.find({ user: { $in: favoriteIds } })
    ...
    .populate(
      "vehicles",
      "vehicleName vehicleType brand model status approvalStatus",
    )
  ...
  const profileByUser = new Map(
    profiles
      ...
      .map((profile) => [String(profile.user._id), profile]),
  );
  const ordered = favoriteIds
    .map((id) => profileByUser.get(String(id)))
    .filter(Boolean);
...
export const getDashboard = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    Lesson.find({
      ...
      .populate({
        path: "booking",
        populate: { path: "offer", select: "title category" },
      })
      .lean(),
    ...
    QuizAttempt.aggregate([
      { $match: { student: userId } },
      {
        $group: {
          ...
          average: {
            $avg: {
              $cond: [{ $eq: ["$status", "completed"] }, "$percentage", null],
            },
          },
          totalAttempts: { $sum: 1 },
        },
    ...
    Exam.findOne({ student: userId, examType: "driving" })
      .sort({ createdAt: -1 })
      .lean(),
  ]);
  ...
  const mapLesson = (lesson) => ({
    ...
    instructorName:
      lesson.teacher?.name || lesson.teacher?.fullName || "Instructor",
    teacher: lesson.teacher,
    vehicleType:
      lesson.booking?.vehicleType ||
      lesson.booking?.vehicle?.vehicleType ||
      "automatic",
  ...
  const settingValues = new Map(
    journeySettings.map((item) => [item.key, item.value]),
  );
  const drivingTargetHours = Number(
    settingValues.get("requiredDrivingHours") || 20,
  );
  const targetSkillsPercentage = Number(
    settingValues.get("requiredSkillsPercentage") || 60,
  );
  ...
  const drivingProgress = Math.min(
    Math.round((completedHours / drivingTargetHours) * 100),
    100,
  );
  const examProgress =
    drivingExam?.status === "passed"
      ? 100
      : drivingExam?.status === "scheduled"
        ? 50
        : 0;
  ...
  const documentsApproved =
    approvedRequiredDocuments === requiredDocumentKeys.length;
  ...
  const profileCompletionItems = [
    {
      ...
      label: "Account Setup",
      weight: 10,
      completed: [registrationUser?.name, registrationUser?.email].every(
        hasValue,
      ),
    },
    {
  ...
  const journeySteps = [
    {
      code: "registration",
      label: "Driving registration",
      progress: registrationComplete ? 100 : 0,
      completed: registrationComplete,
    },
    {
      code: "first_lesson",
      label: "First lesson",
      progress: firstLessonComplete ? 100 : 0,
      completed: firstLessonComplete,
    },
    {
  ...
  const currentStepIndex =
    firstIncompleteStep === -1 ? journeySteps.length - 1 : firstIncompleteStep;
  ...
  return sendResponse(res, 200, "Student dashboard fetched.", {
    ...
    licenseJourney: {
      ...
      currentStepLabel:
        journeySteps[currentStepIndex]?.label || "License Completed",
      completedHours,
      targetHours: drivingTargetHours,
      skillsPercentage: bookletAverage,
      targetSkillsPercentage,
      overallProgress: Math.round(
        journeySteps.reduce((sum, step) => sum + step.progress, 0) /
          journeySteps.length,
      ),
