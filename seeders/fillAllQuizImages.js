...
const categoryFor = (question) => {
  ...
  if (
    topic === "HAS" ||
    /first aid|injur|accident|emergency stop/i.test(text)
  ) {
    return "firstAid";
  }
  if (/park|parking|bus stop|loading zone|footpath/i.test(text))
    return "parking";
  if (
  ...
  if (
    topic === "M" ||
    /brake|tyre|tire|steering|mechanical|engine/i.test(text)
  ) {
    return "mechanical";
  }
  if (
    topic === "S" ||
    /seatbelt|child safety|safety equipment|headlight/i.test(text)
  ) {
    return "vehicle";
  }
  if (
    topic === "U" ||
    /pedestrian|cyclist|motorcycle|animal|road user/i.test(text)
  ) {
    return "roadUsers";
  }
  if (topic === "E" || /fuel|emission|eco|environment/i.test(text))
    return "ecology";
  if (topic === "L") return "regulations";
  if (topic === "R" || /road|weather|fog|night|hydroplan/i.test(text))
    return "road";
  return "driver";
};
